...
// EMS/SCADA interest form endpoint
app.post('/api/solutions-interest', async (req, res) => {
  try {
    const {
      companyName,
      contactPerson,
    ...
    console.log('📥 New EMS/SCADA interest form submission received:', {
      companyName,
      contactPerson,
      email,
      parkType,
    ...
    if (!companyName || !contactPerson || !email || !phone || !parkType || !location || !powerMw || !capacityMwh || !connectionTerms) {
      throw new Error('Missing required fields for EMS/SCADA interest form');
    }
    ...
    <head>
      <meta charset="utf-8">
      <title>New EMS/SCADA Interest Form - RENER ENERGY</title>
      <style>
        body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; }
        .header { background-color: #1d4ed8; color: white; padding: 20px; text-align: center; }
        .content { padding: 20px; }
        .section-title { margin: 20px 0 10px; color: #1d4ed8; }
        .field-table { width: 100%; border-collapse: collapse; margin: 10px 0 20px; }
        .field-table td { border: 1px solid #ddd; padding: 12px; }
        .field-table .label { background-color: #f8f9fa; font-weight: bold; width: 220px; }
        .message-box { background-color: #f9fafb; padding: 15px; border-left: 4px solid #1d4ed8; margin-top: 10px; }
        .footer { background-color: #f8f9fa; padding: 10px; text-align: center; font-size: 12px; color: #666; }
      </style>
    </head>
    <body>
      <div class="header">
        <h1>🧾 EMS / SCADA Interest Form</h1>
        <p>New potential customer submission</p>
      </div>

      <div class="content">
        <h3 class="section-title">A. Company Information</h3>
        <table class="field-table">
        ...
        <h3 class="section-title">B. Park Information</h3>
        <table class="field-table">
          <tr><td class="label">Type</td><td>${parkType}</td></tr>
          <tr><td class="label">Location</td><td>${location}</td></tr>
          <tr><td class="label">Power (MW)</td><td>${powerMw}</td></tr>
          <tr><td class="label">Capacity (MWh)</td><td>${capacityMwh}</td></tr>
          <tr><td class="label">Connection Terms</td><td>${connectionTerms}</td></tr>
        </table>

        <h3 class="section-title">Additional Information / Comments</h3>
        <div class="message-box">
          ${comments ? comments.replace(/\n/g, '<br>') : 'No additional comments provided.'}
        </div>

        <p><strong>Submitted:</strong> ${new Date().toLocaleString('el-GR', { timeZone: 'Europe/Athens' })}</p>
      </div>

      <div class="footer">
        <p>This email was automatically generated by the RENER ENERGY Solutions Interest Form.</p>
      </div>
    </body>
    </html>
    `;
    ...
    const mailOptions = {
      from: `"${contactPerson}" <${senderEmail}>`,
      to: recipientEmail,
      cc: process.env.CC_EMAIL || undefined,
      replyTo: email,
      subject: `EMS/SCADA Interest: ${companyName} (${parkType})`,
      html: emailHTML,
      headers: {
        'X-Application-Source': 'RENER-ENERGY-SOLUTIONS-INTEREST',
        'X-Contact-Email': email,
        'X-Company-Name': companyName,
        'X-Park-Type': parkType,
        'X-Mailer': 'RENER Solutions Interest Form',
        'Sender': senderEmail
      }
    };

    const emailResult = await transporter.sendMail(mailOptions);
    console.log('✅ EMS/SCADA interest email sent successfully:', emailResult.messageId);
    ...
    console.error('❌ EMS/SCADA interest submission failed:', error);
    if (error.message.includes('Missing required fields')) {
    ...
    res.status(500).json({
      success: false,
      error: 'Unable to process interest form. Please try again later.'
    });
  }
});
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('   • POST /api/solutions-interest - EMS/SCADA interest form');
